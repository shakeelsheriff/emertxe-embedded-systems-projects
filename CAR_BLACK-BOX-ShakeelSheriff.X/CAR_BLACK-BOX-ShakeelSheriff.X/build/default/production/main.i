...
void main(void) {
    ...
    unsigned char screen_flag = 0, reset_flag, menu_pos;
    record_event(event, speed);
    eeprom_write_string(0x00, "1010");
    while (1) {
        ...
        if (key == 0x3E) {
            ...
            strcpy(event, gear[gr]);
            record_event(event, speed);
        } else if ((key == 0x37 || key == 0x2F) && screen_flag == 0) {
            screen_flag = 1;
            clear_screen();
            ...
            reset_flag = 0x02;
            TMR2ON = 1;
        } else if ((key == 0x80) && screen_flag == 4) {
            switch (menu_pos) {
                case 0:
        ...
        else if (key == 0x80 && screen_flag == 8) {

            screen_flag = 4;

            clear_screen();
        }
        else if (key == 0x80 && screen_flag == 80) {
        ...
        else if (key == 0x90 && screen_flag == 4) {
        ...
        switch (screen_flag) {
            ...
            case 8:
                ...
                if(view_records(key, reset_flag) == 0x33){
                    clear_screen();
                    screen_flag = 0;
                    reset_flag = 0x05;
                }
            ...
            case 10:
                ...
                if (clear_records(reset_flag) == 0x11)
            ...
            case 40:
                ...
                if (set_time(key, reset_flag) == 0x11) {
                    screen_flag = 4;
                    reset_flag = 0x05;
                    clear_screen();
                    continue;
