...
# 1 "./External_EEPROM_AT24C04.h" 1
# 12 "./External_EEPROM_AT24C04.h"
...
void store_records_EEPROM(void) {
    static unsigned char addr = 0x05;
    pos++;
    if (pos == 10) {
        pos = 0;
    }
    addr = (pos * 10) + 0x05;
    eeprom_write_string(addr, records);
    if (record_count < 9) {
        record_count++;
    }
}
...
void record_event(unsigned char event[], unsigned char speed) {
    get_time();
    strncpy(records, time, 6);
    strncpy(&records[6], event, 2);
    records[8] = speed / 10 + '0';
    records[9] = speed % 10 + '0';
    records[10] = '\0';
    store_records_EEPROM();
}
...
unsigned char login(unsigned char key, unsigned char reset_flag) {
    ...
    if (i == 4) {
        unsigned char saved_pwd[4];
        for (int j = 0; j < 4; j++) {
            saved_pwd[j] = eeprom_read(j);
        }
        ...
        if (strncmp(usr_pwd, saved_pwd, 4) == 0) {
            ...
            clcd_print("Login Success", (0x80 + 2));
...
unsigned char view_records(unsigned char key, unsigned char reset_flag) {
    static char record_number;
    unsigned char records[11];
    int record_pos;
    unsigned char addr;
    ...
    if (record_count == -1) {
        ...
        if (key == 0x37 && record_number > 0) {
            record_number--;
        }
        else if (key == 0x37 && record_number == 0) {
            record_number = record_count;
        }
        else if (key == 0x2F && record_number < record_count) {
            record_number++;
        }
        else if (key == 0x2F && record_number == record_count) {
            record_number = 0;
        ...
        record_pos = record_number;


        for (int i = 0; i < 10; i++) {
            addr = (record_pos * 10) + 5 + i;
            records[i] = eeprom_read(addr);
        }
        records[10] = '\0';
        clcd_putch(record_number + '0', (0xC0 + 0));


        clcd_putch(records[0], (0xC0 + 2));
        clcd_putch(records[1], (0xC0 + 3));
        clcd_putch(':', (0xC0 + 4));


        clcd_putch(records[2], (0xC0 + 5));
        clcd_putch(records[3], (0xC0 + 6));
        clcd_putch(':', (0xC0 + 7));


        clcd_putch(records[4], (0xC0 + 8));
        clcd_putch(records[5], (0xC0 + 9));


        clcd_putch(records[6], (0xC0 + 11));
        clcd_putch(records[7], (0xC0 + 12));


        clcd_putch(records[8], (0xC0 + 14));
        clcd_putch(records[9], (0xC0 + 15));
...
unsigned char set_time(unsigned char key, unsigned char reset_flag) {
    ...
    clcd_putch((char) new_time[0] / 10 + '0', (0xC0 + 0));
    clcd_putch(new_time[0] % 10 + '0', (0xC0 + 1));
    clcd_putch(':', (0xC0 + 2));


    clcd_putch((char) new_time[1] / 10 + '0', (0xC0 + 3));
    clcd_putch(new_time[1] % 10 + '0', (0xC0 + 4));
    clcd_putch(':', (0xC0 + 5));


    clcd_putch((char) new_time[2] / 10 + '0', (0xC0 + 6));
...
unsigned char password_change(unsigned char key, unsigned char reset_flag) {
    ...
    static char usr_pwd[4];
    static char re_usr_pwd[4];
    static char pwd_changed = 0;
    static unsigned int pass_pos;
    static unsigned int re_pass_pos;
    ...
    if (stage == 0) {
        if (key == 0x37 && pass_pos < 4) {
            usr_pwd[pass_pos] = '1';
            clcd_putch('*', (0xC0 + 6 + pass_pos));
            pass_pos++;
            return_time = 5;
        } else if (key == 0x2F && pass_pos < 4) {
            usr_pwd[pass_pos] = '0';
            clcd_putch('*', (0xC0 + 6 + pass_pos));
            pass_pos++;
            return_time = 5;
        ...
        switch (key) {
            case 0x2F:
                re_usr_pwd[re_pass_pos] = '0';
                clcd_putch('*', (0xC0 + 6 + re_pass_pos));
                re_pass_pos++;
                return_time = 5;
                break;
            case 0x37:
                re_usr_pwd[re_pass_pos] = '1';
                clcd_putch('*', (0xC0 + 6 + re_pass_pos));
                re_pass_pos++;
                return_time = 5;
                break;
        ...
        if (re_pass_pos == 4) {
            ...
            if (strncmp(usr_pwd, re_usr_pwd, 4) == 0) {

                eeprom_write_string(0x00, &usr_pwd[4]);
                pwd_changed = 1;
                clear_screen();
                clcd_write(0x0C, 0);
                _delay((unsigned long)((100)*(20000000/4000000.0)));
