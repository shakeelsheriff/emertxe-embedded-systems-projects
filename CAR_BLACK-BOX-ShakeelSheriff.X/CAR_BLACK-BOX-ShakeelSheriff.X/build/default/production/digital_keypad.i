...
unsigned char read_digital_keypad(unsigned char mode) {
    static unsigned char key_state = 0x3F;
    ...
    if (mode == 0) {
        ...
        if (key != 0x3F && key_state == 0x3F)
        {
            key_state = key;
            press_duration = 0;
            long_press_detected = 0;
        } else if (key == 0x3F && key_state != 0x3F)
        {
            if (press_duration < 10 && !long_press_detected)
            {
                unsigned char short_key_press = key_state;
                key_state = 0x3F;
                return short_key_press;
            } else
            {
                key_state = 0x3F;
                press_duration = 0;
